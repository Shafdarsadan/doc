    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
              onPressed: () async {
                point.value = await Navigator.push(
                    context,
                    MaterialPageRoute(
        ...
        child: Obx(() {
          ...
          if (point.value != null) {
            ...
            var newPlusPoint = GeoPoint(
                point.value!.latitude + (radiusInKM / 111.12),
                point.value!.longitude +
                    (radiusInKM / (111.12 * cos(point.value!.latitude))));
            var newMinusPoint = GeoPoint(
                point.value!.latitude - (radiusInKM / 111.12),
                point.value!.longitude -
                    (radiusInKM / (111.12 * cos(point.value!.latitude))));
            print("${newMinusPoint.longitude},${newMinusPoint.latitude}");
            print("${newPlusPoint.longitude},${newPlusPoint.latitude}");
            future = future
                .where("location", isGreaterThanOrEqualTo: newMinusPoint)
                .where("location", isLessThanOrEqualTo: newPlusPoint);
          }
